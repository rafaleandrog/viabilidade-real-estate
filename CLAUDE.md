...
## Estado do backlog — ✅ RODADA 13 CONCLUÍDA (2026-09-25) · ✅ RODADA 14 CONCLUÍDA (2026-09-25)
...
| **13 — Tornado de alavancas e margem de segurança (aba Cenários do Preliminar)** | A **Fase 2** do mesmo handoff de UX da Rodada 12, que aquela rodada registrou como fora de escopo (`historico/rodada-12/auditoria.md`, § 2.7 e § "O que a Fase 0 NÃO cobre" — as duas notas hoje datadas como superadas): tornado de alavancas ranqueadas (§4.2), bloco de margem de segurança (§4.3) e a aba Cenários reconstruída (§4.6) — antes da rodada, um dropdown de UMA variável mais três colunas de valores absolutos em `_renderSensibilidade` (`frontend/tela-proforma.ts`). ⚠️ A auditoria da Rodada 12 supôs que a margem de segurança exigiria um campo `base_calculo` por linha de custo; o plano da 13 **inverteu o motor numericamente** (secante + bisseção, o precedente é `precoSugeridoM2` em `frontend/proforma.ts`), então **nenhuma PR desta rodada mexeu em `schema.json` nem bumpou a `versao`**. Fila de 12 PRs, estritamente serial; plano em `historico/rodada-13/planejamento.md` | **#724–#736** (13), das quais **12 na fila** — a **#724** (custo de licenciamento digitado, salvo e que **nunca entra no proforma**) é bug de motor achado de passagem no planejamento, com dinheiro dentro, e fica **fora da fila**: é decisão do autor quando entra | ✅ **concluída em 2026-09-25** — os 2 PRs de **processo** mergeados primeiro (#737 adotou o **Kimi** como segundo motor externo de revisão; #739 criou o **corpo de conhecimento das lentes**, lido da BASE). Produto em seis PRs: **757** (#725, #727, #728, #729, #732, #733 — tornado, passo, margem de segurança), **774** (#730, tabela com Δ% e invariantes recolhidas), **775** (#731, faixa contra benchmark), **776** (#734, consumo do colchão — a revisão Kimi achou um bloqueante real, base deficitária lida como consumo, consertado antes do merge), **777** (#735, cenário composto com motor multivariável) e o de fechamento (#736: `historico/rodada-13/planejamento.md`, esta linha, as duas notas envelhecidas de `historico/rodada-12/auditoria.md` e o `PROGRESSO.md`). A **#724** fechou fora da fila pelo ramo (b) no PR 772 (nunca houve tela). **Fica aberta só a #726** (decisão do autor: muda número existente). Fora, com motivo em `historico/rodada-13/planejamento.md`: Cenários do Avançado, benchmark por praça, indicadores físicos, Fase 4. O App do Codex ficou sem cota a partir do PR 763 e todas as revisões saíram por lentes Kimi; merge autorizado pelo autor para a sessão inteira. |
...
> `frontend/fluxo-caixa-motor.ts:1763-1764` e `calcularFluxo` agrega juros, principal, carteira e
...
4. **Escrever o corpo do PR num arquivo e rodar o preflight** — `node scripts/preflight-pr.mjs
   ...
   > um passo ao `processo-integro` acrescenta a linha no preflight junto, senão esta frase volta a
   > ser falsa — e ela já foi: um comentário chegou a declarar a lacuna fechada quando três baterias
   > ainda estavam de fora. Achado de lente.
...
| `testar-corpus-revisao.mjs` | preflight + CI (`processo-integro`) | Que o corpo **viaje de fato**: marcador sincronizado, ordem de leitura no briefing, `${COMUM}` interpolado no prompt dos dois motores, `CORPUS:` em todo template de saída — e **extrai do doc e EXECUTA** o comando que lê o marcador | Não julga o conteúdo do corpo, e não roda a lente |
| `testar-colheita-motor.mjs` | preflight + CI (`processo-integro`) | Que a saída de cada lente seja lida pelo parser do motor **certo**, e que lente que morreu vire **falha visível** em vez de sumir do relatório | Não julga o achado da lente, só se ele chegou a ser colhido |
...
- **Todo valor monetário resultado de fórmula tem 2 casas decimais** — na apresentação, na entrada e
  ...
  > duas derivadas saem em módulo — sem `(0)` nem `-0,0%`, e com a magnitude intacta —, porque a
  > classe de sinal é da LINHA e segue o R$ (achado do App do Codex na revisão). (c) Não alcança o Fluxo de Caixa
