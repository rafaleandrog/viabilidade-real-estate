...
| 8 | ✅ Implementado | #23 feito — aba **Resumo** consolidada (`viab-tela-resumo`): 8 KPIs (VPL/TIR/Payback/Exposição + VGV/Resultado/Margem líq./ROI) + 4 gráficos (Fluxo Acumulado, Fluxo Mensal, Composição de custos, Medidores vs. benchmark). Gráficos de fluxo extraídos p/ `fluxo-graficos.ts` (reuso). Frontend puro; verde |
