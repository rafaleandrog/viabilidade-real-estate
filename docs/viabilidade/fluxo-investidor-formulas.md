...
`simularDivida` (`funding-motor.ts:252`).
...
`fundingDoEstudo` (`funding-motor.ts:761`). Redigitar criaria uma segunda fonte de verdade,
...
Implementação: `simularEquity` (`funding-motor.ts:441`).
...
Implementação: `simularFinanciamentoProducao` (`funding-motor.ts:327`) — a matemática da §4.3 foi
...
`indicadoresOperacao` (`funding-motor.ts:538`) devolve, na visão do investidor: investimento total
...
A costura é `FundingNoFluxo` (`funding-motor.ts:706`), criada pela #349 e preservada de propósito
