...
> `area_pvt_r_fechada` / `area_pvt_nr_fechada` (`frontend/proforma.ts:345`), convertida por
> `converterUnidade`/`paraBase` (`frontend/premissas-conversao.ts`); a ALV da cascata
> (`CASCATA_LOTEAMENTO`) só existe no **Loteamento** (`frontend/proforma.ts:341`, dentro do
...
>
> `permutaFinanceiraDeduzidaMensal` (`frontend/fluxo-caixa-motor.ts:1996-2010`) **subtrai** cada
...
Incorporação é `area_pvt_r_fechada`/`area_pvt_nr_fechada` (`frontend/proforma.ts:345`); a ALV da
...
as 4 parcelas PVT (abertas **e** fechadas). O VGV (**C1**/**A2**) só soma as fechadas. Um projeto com
bastante área aberta pode aparecer **sob o teto para o VGV** e **estourando o aproveitamento** ao
