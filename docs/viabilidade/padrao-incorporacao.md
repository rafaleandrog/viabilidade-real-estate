...
> `area_pvt_r_fechada` / `area_pvt_nr_fechada` (`frontend/proforma.ts:337`), convertida por
> `converterUnidade`/`paraBase` (`frontend/premissas-conversao.ts`); a ALV da cascata
> (`CASCATA_LOTEAMENTO`) só existe no **Loteamento** (`frontend/proforma.ts:333`, dentro do
...
Incorporação é `area_pvt_r_fechada`/`area_pvt_nr_fechada` (`frontend/proforma.ts:337`); a ALV da
