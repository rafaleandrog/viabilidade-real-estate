// Render de <viab-grafico-barra-ranqueada> standalone (Rodada 12, PR4).
//
// ⚠️ Transbordo de TEXTO/corte por overflow oculto reportado e não
// asseverado, mesmo padrão de `tabela-fluxo.render.test.ts` — coluna de
// rótulo estreita (~112px) de propósito, com elipse + `title` (tooltip).
...
test('Barra horizontal ranqueada: as linhas, trilhos e barras chegam à tela', { skip: pular ?? false }, async () => {
  const a = await verificarRender({ caso: 'grafico-barra-ranqueada' });
