...
import {
  simularDivida, simularEquity, indicadoresOperacao, fundingDoEstudo,
...
// em docs/viabilidade/fluxo-investidor-formulas.md §4.
//
...
test('#355 fundingDoEstudo: Financiamento à produção usa a mesma matemática da Dívida (D12)', () => {
  const prazo = PRAZO_DIVIDA;
  const livre = new Array(prazo).fill(0);
  ...
  assert.deepEqual(comoFin!.operacoes[0].saidas, comoDiv!.operacoes[0].saidas);
  assert.deepEqual(comoFin!.operacoes[0].saldo, comoDiv!.operacoes[0].saldo);
});
