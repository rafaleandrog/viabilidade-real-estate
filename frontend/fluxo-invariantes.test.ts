...
// #584 (era #462): `validarContratacao` continua verde com área aberta > 0 —
// este validador tem a MESMA fórmula de `vgvVendavelLinha`, e é ele quem
// denuncia se a retirada do deflator ficar pela metade: se um dos dois
// caminhos ainda deflacionar a área aberta, este teste reprova.
test('#584: validarContratacao reconcilia com área aberta a preço cheio (fechada 50 + aberta 10 = 60 m²/un.)', () => {
  const linhas = [{
    ...RECEITA_PRODUTO[0],
    tipologias: [{
  ...
  // VGV = 20 × (50 + 10) × 10.000 = 20 × 600.000 = 12.000.000
  // (sem área aberta seria 10.000.000; com o deflator de 50% da #462 seriam
  // 11.000.000 — a diferença prova que a área aberta entra INTEIRA).
  assert.deepEqual(validarContratacao(linhas, CRONO_PRODUTO, 20, 12_000_000, undefined, []), []);
  const div = validarContratacao(linhas, CRONO_PRODUTO, 20, 11_000_000, undefined, [])[0];
  assert.equal(div.codigo, 'VENDA_BRUTA_NAO_RECONCILIA');
  assert.equal(div.diferenca, -1_000_000);
});
...
test('#445 validarFunding: (a) modo resultado_final com resultadoFinal negativo produz retorno negativo', () => {
  // funding-motor.ts:591 — `saidas[t] = round2(resultadoFinal * pct)`, sem
