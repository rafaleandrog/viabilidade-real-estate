...
export function kpisDoCaso(config: FluxoConfig, operacoes: OperacaoFunding[]): KpisBaseline {
  ...
  // #604/#611: `margemPct` e `roiPct` viraram `number | null`. Aqui não se usa
  // `?? 0`: um baseline de catraca com KPI INDEFINIDO não é um baseline —
  // seria uma linha comparando 0 contra 0 e passando para sempre. Todo estudo
  // desta fixture tem Receita Bruta > 0 e investimento > 0; se algum dia
  // deixar de ter, é para explodir ruidosamente aqui, não para virar zero em
  // silêncio.
  if (p.margemPct === null) {
