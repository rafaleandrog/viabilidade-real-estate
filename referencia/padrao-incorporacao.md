...
> `CASCATA_INCORPORACAO` (`frontend/areas-cascata.ts:232-237`) **é renderizada** pela aba Terreno &
...
| **C7** | **Todo valor monetário resultado de fórmula tem 2 casas decimais** — na apresentação, na entrada e no motor. O **valor canônico** de uma premissa multiunidade é o monetário; `% do VGV` e `R$/m²` são representações **derivadas**, que carregam precisão plena internamente e arredondam só para exibir. Contrato do autor, 2026-08-01. **Estado:** `fmtR$` (`viab-format.ts:11-23`) usa 2 casas, a exportação passou a importá-lo em vez de definir formatador próprio (`exportar.ts:16`) e a sensibilidade da Proforma migrou (`tela-proforma.ts:81`, `celulaSensibilidade`, #492). **A #281 fechou com a #449** (2026-08-24): `fluxo-tabela.ts:40` e `tela-fluxo-receitas.ts:451-452` (`precoUnit`/`precoTotal`) delegam para `celula`/`fmtR$` de `viab-format.ts`; `celulaProformaM2` (`tela-proforma.ts:52`, extraída de método privado pela #567) é a R$/m² — derivada não monetária, `fmtNum` sem casas com a mesma regra de sinal (`negativoContabil`). O **valor canônico** — persistido, digitado e calculado — segue em 2 casas em todo lugar; a **exibição** tem exatamente **três exceções declaradas**, abaixo, e nenhuma outra. Tabela completa (com evidência arquivo:linha) em `formulas.md`. **⚠️ Primeira exceção, do autor em 2026-08-26 (#581):** valor em R$ exibido em **card de KPI** sai **sem casas decimais** (percentual em card, com **uma**), por `fmtR$Kpi` (`frontend/viab-format.ts:52`) — é exibição, não dado: persistência, entrada, motor, Fluxo de Caixa e exportação do Fluxo seguem em 2 casas (a Proforma tem a sua própria exceção, a terceira). **Segunda exceção (2026-09-15):** o rótulo de barra da cascata do resultado sai em R$ milhões com uma casa (`fmtR$Milhoes`). **Terceira exceção (#754, 2026-09-18):** a **coluna R$ da Proforma** — Preliminar e Avançado, tela, sensibilidade, CSV e PDF — sai em **inteiros**, por `celulaInteira` (`frontend/viab-format.ts`), chamada por `celulaProforma` (`exportar.ts:76`, reexportada pela tela — na #449 ela delegava para `celula`, e desde a #754 chama `celulaInteira`) e pela Proforma do Avançado (`tela-fluxo-ver.ts`); as colunas derivadas R$/m² e % VGV da mesma linha herdam o sinal do R$ publicado (`semZeroNegativo`), sem mudar de precisão; é a única exceção que alcança uma tabela e a sua exportação, porque tela e arquivo compartilham a função e a paridade entre eles é contrato. Trava: `frontend/proforma-inteiros.test.ts`. | `formulas.md`, `viab-format.ts`, `premissas-conversao.ts` |
